...
<xsl:transform version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:variable name="ourLevelX"
        select="$firstLevelX + (floor($addedLevels div $levelsPerColumn) * $offsetX)" />
    <xsl:variable name="ourLevelY"
        select="$firstLevelY + (($addedLevels mod $levelsPerColumn) * $offsetY)" />
    ...
    <xsl:template match="/scene/buttongroup[@id='levelMarkerGroup']">
        <xsl:copy>
            ...
            <button id="lb_{$level_id}" depth="0" x="{$ourLevelX}" y="{$ourLevelY}" scalex="1.008"
                scaley="0.848" rotation="-0.5" alpha="1" colorize="255,255,255"
                up="IMAGE_SCENE_ISLAND1_LEVELMARKERA_UP"
                over="IMAGE_SCENE_ISLAND1_LEVELMARKERA_OVER" onclick="pl_{$level_id}" />
    ...
    <xsl:template match="/scene">
        <xsl:copy>
            ...
            <label id="txt_{$level_id}" depth="0.1" x="{$ourLevelX + 30}" y="{$ourLevelY}"
                align="left" rotation="6.337" scale="0.7" overlay="false" screenspace="false"
                font="FONT_INGAME36" text="{$level_name_id}" />

            <SceneLayer id="ocd_{$level_id}" name="OCD_flag1" depth="-0.1" x="{$ourLevelX}"
                y="{$ourLevelY + 20}" scalex="0.7" scaley="0.7" rotation="17.59" alpha="1"
                colorize="255,255,255" image="IMAGE_SCENE_ISLAND1_OCD_FLAG1" anim="ocdFlagWave"
                animspeed="1" />
