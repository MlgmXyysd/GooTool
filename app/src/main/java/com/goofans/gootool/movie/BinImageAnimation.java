 ...
 * @version $Id: BinImageAnimation.java 389 2010-05-02 18:03:02Z david $
 */
...
//          if (hasColor && colorFrames[frame].interpolationType != transformFrames[0][frame].interpolationType) throw new AssertionError(this);
            ...
            for (int i = 0; i < transformTypes.length; i++) {
                ...
                for (int j = 0; j < transformFrames[i].length; j++) {
                    ...
                    if (frame != null) {
                        if (transformType == TransformType.SCALE && frame.x == -1)
                            throw new AssertionError(frame);
                        if (transformType == TransformType.ROTATE && frame.x != -1)
                            throw new AssertionError(frame);
                        if (transformType == TransformType.TRANSLATE && frame.x == -1)
                            throw new AssertionError(frame);
                        if (transformType == TransformType.SCALE && frame.y == -1)
                            throw new AssertionError(frame);
                        if (transformType == TransformType.ROTATE && frame.y != -1)
                            throw new AssertionError(frame);
                        if (transformType == TransformType.TRANSLATE && frame.y == -1)
                            throw new AssertionError(frame);
                        if (transformType == TransformType.ROTATE && frame.angle == -1)
                            throw new AssertionError(frame);
                        if (transformType != TransformType.ROTATE && frame.angle != -1)
                            throw new AssertionError(frame);
                        ...
                        if (frame.interpolationType != 0 && frame.interpolationType != 1)
                            throw new AssertionError(frame);
                    }
                }
            }
